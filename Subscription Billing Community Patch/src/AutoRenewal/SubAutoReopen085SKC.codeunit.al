...
/// when AutoRenewal085SKC is enabled.
///
/// Real-time path: OnAfterModifyEvent on Subscription Line detects the close.
///   Auto-renewal lines: captures Term Until, then immediately re-opens the line.
///   Non-auto-renewal lines: captures Term Until into TermUntilBackup085SKC only.
...
/// </summary>
codeunit 70631066 SubAutoReopen085SKC
{
    Access = Internal;
    ...
    begin
        ...
        if not Rec.AutoRenewal085SKC then begin
            SubLine.TermUntilBackup085SKC := TermUntilToRestore;
    ...
    begin
        if SubLine.Closed then
            exit;
        if not SubLine.AutoRenewal085SKC then
            exit;
    ...
    begin
        ...
        ReopenSubscriptionLine(SubLine, SubLine.TermUntilBackup085SKC);
    end;
    ...
    begin
        ...
        SubLine.TermUntilBackup085SKC := 0D;
        SubLine.Modify(false);
    ...
    /// Falls back to the current "Term Until" when TermUntilBackup085SKC
    ...
    begin
        ...
        SubLine.SetRange(AutoRenewal085SKC, true);
    ...
    begin
        ...
        VendContractLine.SetRange("Subscription Line Entry No.", SubLine."Entry No.");
        VendContractLine.SetRange(Closed, true);
        if not VendContractLine.IsEmpty() then
            VendContractLine.ModifyAll(Closed, false, false);
    end;
